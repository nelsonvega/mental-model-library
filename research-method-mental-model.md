---
noteId: "1fcfd77039d811f1bdb23db937d9dbc4"
tags: []
...
2. Clarify scope using **Circle of Competence**:
    - List: “What do I already understand well here?” vs. “What is fuzzy / unknown?”[^1][^2]
...
**Mental model:** First Principles Thinking.[^3][^4][^5][^1]
...
2. **Interrogate each one with “Why?” and “How do we know?”**
    - Ask “Why is this true?” repeatedly until you hit something that would be very hard to dispute without new evidence (physics, math, identities, hard constraints, definitions, regulatory facts, etc.).[^4][^5][^1]
3. **Separate:**
...
**Mental models:** Pareto Principle (80/20), Inversion, Via Negativa.[^6][^7][^8]

**Goal:** Decide what’s worth researching *deeply*.

1. **80/20 Scan:**
    - Which 20% of variables explain 80% of outcomes? (e.g., in an options strategy: IV surface behavior, liquidity, margin rules).[^6]
2. **Inversion:**
    - Ask “If this project completely failed, what were the 3 most likely reasons?”[^8]
    - Research those failure modes early (regulation, liquidity, data quality, model risk).
3. **Via Negativa (removal instead of addition):**[^7]
...
**Mental models:** Reasoning by analogy (deliberate), Map vs. Territory, Second‑Order Thinking.[^2][^1][^8]

**Goal:** Generate non‑obvious ideas without abandoning rigor.

1. **Analogy harvesting:**
    - “Where else does a system like this exist?”
    - E.g., order books ↔ queuing systems; membership churn ↔ survival analysis; construction workflows ↔ manufacturing pipelines.[^2]
2. **Borrow and adapt:**
    - For each analog domain, list 2–3 techniques that might port over.
    - Explicitly write: “Same?” / “Different?” so you don’t over‑fit the analogy.[^1][^2]
3. **Map vs. Territory check:**
    - Ask: “Is this idea based on a *model* or on *observed data*?”[^1][^2]
    - Force yourself to distinguish theory vs. reality.
4. **Second‑order thinking:**
    - For each candidate idea, ask “And then what?” two steps out—impact on incentives, edge decay, operational load, etc.[^8]
...
**Mental models:** Feedback loops, Testing, Backcasting / Premortem.[^9][^10][^1]

**Goal:** Close the loop so research doesn’t stay abstract.

1. **Backcast from success:**
    - Define: “If this research were wildly successful in 12 months, what would exist? What metrics would be true?”[^9]
...
2. **Design the smallest experiments:**
    - A/B tests, paper trading, small pilot with one association, etc.[^10]
3. **Premortem:**
    - Imagine your experiment failed; list reasons, then adjust design to guard against them.[^9]
4. **Install feedback metrics:**
    - Decide what you’ll measure to update or kill your hypothesis quickly.[^10][^1]
...
1. **Frame:**
    ...
    - What’s inside vs. outside my circle of competence here?[^2][^1]
2. **Decompose (First Principles):**
    - List assumptions → ask “Why?” until you hit fundamentals → capture 5–10 first principles.[^5][^4][^1]
3. **Filter (Applicability):**
    - Identify the 20% of factors that drive 80% of the outcome.
    - Invert: how could this fail? Research those points first.[^7][^6][^8]
4. **Expand (Lateral Thinking):**
    - Collect 2–3 analog domains; harvest candidate techniques.
    - Use map–territory and second‑order thinking to keep ideas grounded.[^8][^1][^2]
5. **Test (Feedback Loops):**
    - Backcast from success → design smallest meaningful experiment.
    - Run a premortem, instrument feedback signals, iterate.[^10][^1][^9]
...
[^3]: https://www.mindlabpro.com/blogs/nootropics/5-mental-models-top-decision-makers-swear-by

[^4]: https://www.extern.com/post/mental-models-for-smarter-thinking-the-first-principles-approach

[^5]: https://www.reddit.com/r/mentalmodels/comments/p0qt2l/mental_model_fundamentals_first_principles/

[^6]: https://www.diygenius.com/powerful-mental-models/

[^7]: https://www.42courses.com/blog/home/2022/4/15/10-more-mental-models-worth-knowing

[^8]: https://www.linkedin.com/posts/vinay-gurram_here-are-5-mental-models-that-changed-how-activity-7417994066690314241-F0Sp

[^9]: https://www.hustleescape.com/mental-models/

[^10]: https://www.convert.com/blog/a-b-testing/mental-models-in-experimentation/

[^11]: https://www.youtube.com/shorts/3rewPpcB1hw
